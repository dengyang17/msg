...
class Example(object):
  ...
  def __init__(self, document, answer_sentences, question, vocab, hps):
    """Initializes the Example, performing tokenization and truncation to produce the encoder, decoder and target sequences, which are stored in self.

    Args:
      document: document text; a list.
    ...
    self.d_lens = []
    self.d_batch = []
    self.ds_lens = len(document) if len(document) <= hps.sent_num else hps.sent_num
    doc_words = []
    for sent in document:
      sent_words = sent.split()
      if len(sent_words) > hps.max_enc_steps:
        sent_words = sent_words[:hps.max_enc_steps]
      doc_words.append(sent_words)
      self.d_lens.append(len(sent_words)) # store the length after truncation but before padding
      self.d_batch.append([vocab.word2id(w) for w in sent_words]) # list of word ids; OOVs are represented by the id for UNK token
    if len(self.d_batch) > hps.sent_num:
      self.d_lens = self.d_lens[:hps.sent_num]
      self.d_batch = self.d_batch[:hps.sent_num]
    ...
    if hps.pointer_gen:
      ...
      self.doc_extend_vocab = []
      for sent_words in doc_words:
        sent_extend_vocab, self.oovs = data.article2ids(sent_words, vocab, self.oovs)
        self.doc_extend_vocab.append(sent_extend_vocab)
      if len(self.doc_extend_vocab) > hps.sent_num:
        self.doc_extend_vocab = self.doc_extend_vocab[:hps.sent_num]
  ...
  def pad_document(self, max_len, sent_num, pad_id):
    """Pad the encoder input sequence with pad_id up to max_len."""
    for i in range(len(self.d_batch)):
      while len(self.d_batch[i]) < max_len:
        self.d_batch[i].append(pad_id)
    while len(self.d_batch) < sent_num:
      self.d_batch.append([pad_id] * max_len)
      self.d_lens.append(0)
    if self.hps.pointer_gen:
      for i in range(len(self.doc_extend_vocab)):
        while len(self.doc_extend_vocab[i]) < max_len:
          self.doc_extend_vocab[i].append(pad_id)
      while len(self.doc_extend_vocab) < sent_num:
        self.doc_extend_vocab.append([pad_id] * max_len)
...
class Batch(object):
  ...
  def init_encoder_seq(self, example_list, hps):
    ...
    max_d_seq_len = max([max(ex.d_lens) for ex in example_list])
    max_q_seq_len = max([ex.q_lens for ex in example_list])

    # Pad the encoder input sequences up to the length of the longest sequence
    for ex in example_list:
      ex.pad_document(max_d_seq_len, hps.sent_num, self.pad_id)
      ex.pad_question(max_q_seq_len, self.pad_id)

    # Initialize the numpy arrays
    # Note: our enc_batch can have different length (second dimension) for each batch because we use dynamic_rnn for the encoder.
    self.d_batch = np.zeros((hps.batch_size, hps.sent_num, max_d_seq_len), dtype=np.int32)
    self.q_batch = np.zeros((hps.batch_size, max_q_seq_len), dtype=np.int32)
    self.d_lens = np.zeros((hps.batch_size, hps.sent_num), dtype=np.int32)
    self.q_lens = np.zeros((hps.batch_size), dtype=np.int32)
    self.ds_lens = np.zeros((hps.batch_size), dtype=np.int32)
    self.d_padding_mask = np.zeros((hps.batch_size, hps.sent_num, max_d_seq_len), dtype=np.float32)
    self.q_padding_mask = np.zeros((hps.batch_size, max_q_seq_len), dtype=np.float32)
    self.ds_padding_mask = np.zeros((hps.batch_size, hps.sent_num), dtype=np.float32)


    # Fill in the numpy arrays
    for i, ex in enumerate(example_list):
      self.d_batch[i, :] = ex.d_batch[:]
      self.d_lens[i, :] = ex.d_lens[:]
      self.q_batch[i, :] = ex.q_batch[:]
      self.q_lens[i] = ex.q_lens
      self.ds_lens[i] = ex.ds_lens
      for j in range(len(ex.d_lens)):
        for k in range(ex.d_lens[j]):
          self.d_padding_mask[i][j][k] = 1
      for j in range(ex.ds_lens):
        self.ds_padding_mask[i][j] = 1
    ...
    if hps.pointer_gen:
      ...
      self.d_batch_extend_vocab = np.zeros((hps.batch_size, hps.sent_num, max_d_seq_len), dtype=np.int32)
      self.q_batch_extend_vocab = np.zeros((hps.batch_size, max_q_seq_len), dtype=np.int32)
      for i, ex in enumerate(example_list):
        self.d_batch_extend_vocab[i, :] = ex.doc_extend_vocab[:]
      for i, ex in enumerate(example_list):
  ...
  def store_orig_strings(self, example_list):
    ...
    self.original_document = [ex.original_document for ex in example_list] # list of lists
...
class Batcher(object):
  ...
  def fill_example_queue(self):
    ...
    while True:
      try:
        (document, answer, question) = next(input_gen) # read the next example from file. article and abstract are both strings.
      ...
      example = Example(document, answer_sentences, question, self._vocab, self._hps) # Process into an Example.
  ...
  def text_generator(self, example_generator):
    ...
    while True:
      ...
      try:
        document = eval(e.features.feature['document'].bytes_list.value[0].decode()) # the article text was saved under the key 'article' in the data files
      ...
      if len(question_text)==0:
        ...
        yield (document, answer_text, question_text)
