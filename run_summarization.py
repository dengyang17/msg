...
# Where to find data
tf.app.flags.DEFINE_string('data_path', 'data/wikihow/chunked/train_*', 'Path expression to tf.Example datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('valid_path', 'data/wikihow/chunked/valid_*', 'Path expression to tf.Example validation datafiles. Can include wildcards to access multiple datafiles.')
tf.app.flags.DEFINE_string('vocab_path', 'data/wikihow/vocab', 'Path expression to text vocabulary file.')
...
tf.app.flags.DEFINE_integer('sent_num', 20, 'max number of reviews for each question.')
...
tf.app.flags.DEFINE_integer('hops', 1, 'number of hops')
...
def run_training(model, batcher, sess_context_manager, sv, summary_writer, vocab, hps):
  ...
  with sess_context_manager as sess:
    ...
    min_loss = 20.0
    #min_loss = run_eval(model, sess, val_batcher)
    t0 = time.time()
    while True: # repeats until interrupted
      ...
      if FLAGS.coverage:
        check_step = 1000
      if train_step % check_step == 0:
        to_be_saved = False
        t1 = time.time()
        tf.compat.v1.logging.info('seconds for training step: %.3f', t1-t0)
        t0 = time.time()
        val_batcher = Batcher(FLAGS.valid_path, vocab, hps, single_pass=True)
        loss = run_eval(model, sess, val_batcher)
        if loss < min_loss:
          min_loss = loss
          to_be_saved = True
        if to_be_saved:
...
def run_eval(model, sess, val_batcher):
  ...
  while True:
    ...
    if FLAGS.coverage:
      ...
      l = results['loss']
    tf.compat.v1.logging.info('validation batch loss: %f', l) # print the accuracy to screen
    loss.append(l)
...
def main(unused_argv):
  ...
  hparam_list = ['mode', 'lr', 'adagrad_init_acc', 'rand_unif_init_mag', 'trunc_norm_init_std', 'max_grad_norm', 'hidden_dim', 'emb_dim', 'sent_num', 'batch_size', 'max_dec_steps', 'max_enc_steps', 'coverage', 'cov_loss_wt', 'pointer_gen', 'glove', 'hops']
  hps_dict = {}
